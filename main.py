...
text_col = st.selectbox("Text column (Complete response)", cols, index=cols.index("Complete Response") if "Complete Response" in cols else 0)
model_col = st.selectbox("Model column", cols, index=cols.index("Model") if "Model" in cols else 0)
prompt_col = st.selectbox("Prompt column (optional)", ["<none>"] + cols)

n_rows = len(df)
row_idx = st.number_input("Response index (0-based)", min_value=0, max_value=max(n_rows - 1, 0), step=1, value=0)
...
with left:
    ...
    st.markdown("---")
    st.markdown(str(row.get(text_col, "")))

with right:
    st.subheader("Ratings")

    # Use a form so we only save when user clicks the button
    with st.form(key=f"rating_form_{row_idx}"):

        # DISCERN
        with st.expander("DISCERN (1–5)", expanded=True):
            for i, text in DISCERN_ITEMS.items():
                key = f"discern_q{i}_{row_idx}"
                default = int(row.get(f"discern_q{i}", 0) or 0)
                st.session_state.setdefault(key, default if default in [1, 2, 3, 4, 5] else 3)
                st.select_slider(
                    f"Q{i}. {text}",
                    options=[1, 2, 3, 4, 5],
        ...
        with st.expander("PEMAT – Understandability (0=Disagree,1=Agree,N/A)", expanded=False):
            for i, text in PEMAT_UNDER_TEXT.items():
                ...
                st.selectbox(
                    ...
                    options=[0, 1, "N/A"],
                    index=[0, 1, 2].index(
                        2 if default == "N/A" else default
                    ) if default in [0, 1, "N/A"] else 0,
                    key=key,
                )
        ...
        with st.expander("JAMA Benchmark (0=No, 1=Yes)", expanded=False):
            for col_name, text in JAMA_ITEMS.items():
                key = f"{col_name}_{row_idx}"
                default = int(row.get(col_name, 0) or 0)
                st.session_state.setdefault(key, default if default in [0, 1] else 0)
                st.radio(
                    text,
                    options=[0, 1],
                    key=key,
        ...
        with st.expander("Overall ratings (1–5)", expanded=False):
            for name, col_name in [
                ...
                key = f"{col_name}_{row_idx}"
                default = int(row.get(col_name, 0) or 0)
                default = default if default in [1, 2, 3, 4, 5] else 3
                st.session_state.setdefault(key, default)
                st.select_slider(
                    name,
                    options=[1, 2, 3, 4, 5],
    ...
    if submitted:
        # Update df with ratings from this row
        ...
        # Recompute DISCERN and PEMAT aggregate scores for this row
        row_after = df.loc[row_idx]
        df.loc[row_idx, list(score_discern(row_after).keys())] = list(score_discern(row_after).values())
        df.loc[row_idx, list(score_pemat(row_after).keys())] = list(score_pemat(row_after).values())
